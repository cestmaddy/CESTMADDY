...
describe('core/scripts/generation/paths.ts', () => {
	describe('getGeneratedPath', () => {
		it('should generate paths', () => {
			...
			// Normal page
			expect(
				getGeneratedPath(path.join(SOURCE_ROOT, 'index.md'), ESourceType.Page)
			).to.equal(
			...
			expect(
				getGeneratedPath(path.join(SOURCE_ROOT, 'source', 'index.markdown'), ESourceType.Page)
			).to.equal(
			...
			expect(
				...
				path.join(base_path, 'source', 'source', 'index.html'),
			);
			...
			expect(
				...
				path.join(base_path, 'blog', 'first_post', 'post.html'),
			);

			// Normal podcast in a SOURCE_ROOT subdirectory
			expect(
				getGeneratedPath(path.join(SOURCE_ROOT, SOURCE_ROOT, 'episode.md'), ESourceType.Episode)
			).to.equal(
			...
			expect(
				getGeneratedPath(path.join(SOURCE_ROOT, 'imgs', 'image.webp'), ESourceType.Other)
			).to.equal(
	...
	describe('getWebPath', () => {
		it('should generate paths', () => {
			// Normal page
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'index.html'), ESourceType.Page)
			).to.equal(
				'/',
			);
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'test', 'index.html'), ESourceType.Page)
			).to.equal(
				'/' + path.join('test'),
			);
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'test'), ESourceType.Page)
			).to.equal(
				'/' + path.join('test'),
			);
			...
			// Normal page in a 'source' subdirectory
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'source', 'index.html'), ESourceType.Page)
			).to.equal(
				'/' + path.join('source'),
			);
			...
			// Normal image in a 'source' subdirectory in a 'source' subdirectory, as a page
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'source', 'source', 'image.png'), ESourceType.Page)
			).to.equal(
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'blog', 'first_post.md'), ESourceType.Post)
			).to.equal(
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'blog', 'first_post', 'post.md'), ESourceType.Post)
			).to.equal(
				'/' + path.join('blog', 'first_post'),
			);
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'podcast', 'first.md'), ESourceType.Episode)
			).to.equal(
			...
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'podcast', 'first', 'episode.md'), ESourceType.Episode)
			).to.equal(
				'/' + path.join('podcast', 'first'),
			);
			...
			// Image
			expect(
				getWebPath(path.join(SOURCE_ROOT, 'imgs', 'image.webp'), ESourceType.Other)
			).to.equal(
	...
	describe('getThemePath', () => {
		it('should generate paths', () => {
			...
			expect(
				...
				path.join(BUILTIN_THEMES_ROOT, 'clean'),
			);
			...
			expect(
				...
				path.join(CUSTOM_THEMES_ROOT, 'test'),
			);
		});
	});
